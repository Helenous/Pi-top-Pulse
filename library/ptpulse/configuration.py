...
def _verify_device_state(expected_state):
    ...
    if expected_state == current_state:
        ...
        PTLogger.warning("Error: Device write verification failed. Expected: " + _get_bit_string(expected_state) + " Received: " + _get_bit_string(current_state))
        return False
...
def enable_device():
    ...
    if is_pi_top_v2:
        ...
        PTLogger.error("Error - unrecognised device ID '" + str(_host_device_id) + "' - unsure how to initialise " + speaker_type_name)
